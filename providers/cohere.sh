...
fetch_with_retry() {
  ...
  while true; do
    ...
    if [[ "$attempt" -ge "$max_attempts" ]]; then
      echo "[cohere] Failed to download $label after $attempt attempts" >&2
      return 1
    fi

    sleep_seconds=$((attempt * 2))
    echo "[cohere] Retry $attempt for $label in ${sleep_seconds}s" >&2
    sleep "$sleep_seconds"
    attempt=$((attempt + 1))
  done
}

echo "[cohere] Downloading llms.txt index from $INDEX_URL"
...
if fetch_with_retry "$FULL_INDEX_URL" "$WORKDIR/llms-full.txt" "llms-full.txt"; then
  ...
  echo "[cohere] Warning: could not download llms-full.txt from $FULL_INDEX_URL" >&2
fi
...
for url in "${URLS[@]}"; do
  rel="${url#https://docs.cohere.com/}"
  dest="$OUT_DIR/$rel"
  mkdir -p "$(dirname "$dest")"
  echo "[cohere] Downloading $rel"
...
echo "[cohere] Downloaded ${#URLS[@]} docs into $OUT_DIR"
